...
- **Banner** : `F2` insère ta bannière (définie dans `lua/personal.lua`, gitignoré), auto-update au save (scopée `*.c`/`*.h`/`*.cpp`/`*.hpp`)
...
| `completion.lua` | nvim-cmp (preselect None, completeopt noinsert/noselect, source buffer retirée) |
| `lsp.lua` | clangd (norme 42, sans placeholders), lua_ls (callSnippet=Disable) |
| `formatting.lua` | conform.nvim (prettierd, prettier ; pas de formatter C/C++ = norme manuelle) |
...
| `headers.lua` | Custom banner from `personal.lua` (autocmd BufWritePre scopé C/C++) |
| `match.lua` | Search/replace flottant custom (~430 lignes Lua, escape proprement, errors visibles) |
| `norminette.lua` | nvim-lint (norminette 42, sortie JSON) |
...
Tes infos (bannière `<F2>` + login 42) vivent dans `lua/personal.lua`, **gitignoré** — elles ne partent pas sur le repo :

```bash
cp lua/personal.lua.example lua/personal.lua
```

Puis édite `lua/personal.lua` : `banner` (ton ASCII/nom/github), `banner_marker`, et `header42` (`user`/`mail` 42). Sans ce fichier : bannière placeholder + login 42 factice.
...
- **Moteur = nvim-cmp** (basculé depuis blink) : `preselect = None` + `completeopt = noinsert,noselect` → naviguer ↑/↓ surligne sans remplacer ton mot ; tu valides avec `<CR>` / `<C-y>`.
- **Source "buffer" retirée** — pas de complétion sur les mots du fichier au hasard.
...
Si Neovim ≥ 0.11 et une Nerd Font installée, ça devrait charger.
