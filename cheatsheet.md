...
|-----|--------|
| `F1` | Header 42 |
| `F2` | Bannière custom (auto-update au save) |
...
> **Prérequis** : binaire avec `-g`. Compile via le terminal flottant `Ctrl+/` : `make debug` ou `make asan`.

## Workflow rapide

1. `Ctrl+/` (terminal flottant) → `make debug` ou `make asan` (recompile avec infos debug)
